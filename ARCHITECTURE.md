...
- **Closed message bus.** The four AceEvent messages (`KickCD_SPELL_STATE`, `KickCD_CONFIG_CHANGED`, `KickCD_PROFILE_CHANGED`, `KickCD_GRID_LAYOUT`) are the only inter-module communication channel. Each has exactly one sender. Adding a message requires updating the source emitter, every consumer, and [docs/message-bus.md](docs/message-bus.md).
...
- **`KickCD.State.inCombat` is the combat flag, not `InCombatLockdown()`.** A bootstrap CreateFrame in `core/State.lua` owns `PLAYER_REGEN_DISABLED/_ENABLED/PLAYER_LOGIN` and maintains the flag. Readers (IconGrid, Castbar) consume it; `InCombatLockdown()` lags the regen events by a frame and is unreliable.
