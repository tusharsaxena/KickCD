...
The settings panel is a six-tab Blizzard subcategory (General / Icons / Cast bar / Text Label / Spells / Profiles) with full slash-command parity (`/kcd help|version|config|lock|unlock|toggle|list|get|set|reset|resetall|resetposition|spells|debug|perf`). Every schema-driven option is automatically wired into UI widget, slash CLI, and Defaults reset — see [docs/settings-panel.md](settings-panel.md).
...
- **Five LibKa0s modules are adopted, one setup file each.** `core/CoreSetup.lua` (`Core` — the secret-safe tagged printer + secret seam), `core/DebugLogSetup.lua` (`DebugLog` — the on-screen console), `core/PerfSetup.lua` (`Perf` — the guided A/B capture behind `/kcd perf`), `settings/Slash.lua` (`Slash` — the `/kcd` dispatcher + schema CLI), `settings/OptionsSetup.lua` (`Options` — the settings panel; the library instance **is** `NS.Settings.Helpers`, decorated in place by `Panel.lua` / `Panel_Widgets.lua` / `Panel_Render.lua`, never copied into a fresh table). **Never edit `libs/`** — a library problem is fixed upstream in `../LibKa0s` and re-vendored, and `diff -r ../LibKa0s/LibKa0s libs/LibKa0s` MUST come back empty. Each setup file owns only what is genuinely this addon's (the tag, the verb table, the schema adapters) and degrades to a stub — never an error — when the library is absent. A descriptor gap you can close inside the setup file is not a library change; say so in a comment rather than forking the library.
- **Colours are stored keyed; dropdown `values` are keyed hashes.** A colour value is `{ r =, g =, b =, a = }` — the library's own vocabulary, so no host-side codec sits between the slash CLI, the panel widget and the DB (the old positional `{r,g,b,a}` array migrated in `Database:MigrateColorShape`, the v3→v4 step). Dropdown `values` are keyed hashes with a sibling `sorting` array. Every schema row carries `desc` (not `tooltip`), `panel` and `section`; every colour row also carries `hasAlpha`.
