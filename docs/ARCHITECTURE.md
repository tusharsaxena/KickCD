...
| `Ka0s_KickCD_CONFIG_CHANGED` | **One sender**: `settings/Panel.lua` `Helpers.FireConfigChanged`. Everything that wants to announce a config change calls it — `Helpers.Set`, `Panel_Render`'s reset helpers, `settings/General.lua`'s Units-tab focus link / copy-styling controls, `core/KickCD.lua`'s lock toggle and spells commit, the Spells editor's throttled commit, and IconGrid / Castbar `OnDragStop` | `IconGrid`, `Cooldowns`, `Castbar`, `UnitLabel`, Spells panel | `{ section }` — section ∈ `general`\|`icons`\|`castbar`\|`label`\|`spells`\|`units` |
...
| `options-ui-§15` | **`General visibility`'s VALUE LIST** is this addon's own — `Always` / `In combat` / `When target is casting` / `When target is casting an interruptible spell` — where §15 mandates `Always` / `Only in combat` / `Only out of combat` / `Never`. Everything else about the row is the composer's: its position, its label, its pairing, its stored path. | KickCD's visibility is **cast-state driven**, and *"when the target is casting an interruptible spell"* is the mode the entire addon exists for — it is what makes an interrupt tracker different from a cooldown bar. The canonical four cannot express it, so adopting them verbatim would delete the feature rather than standardise it. The two canonical modes this addon lacks are covered or meaningless here: `Never` is what the tab's own `Enable KickCD` does, and `Only out of combat` names a state in which nothing this addon draws has anything to show. The stored KEYS are untouched (`in_combat`, not `inCombat`), so this is a declaration difference and not a migration — see [settings-panel.md](settings-panel.md#master-controls--the-canonical-tab-options-ui-15). | 2026-09-02 | `LibKa0s-Options-1.0`'s `MasterControls` composer accepts a `values` override for the visibility row, **or** options-ui-§15 admits an addon-specific visibility set — either makes this a supported shape rather than a departure and retires the row. |
| `options-ui-§1` **(PROVISIONAL — not signed off; see the note below the table)** | The **degraded stub's five schema composers are HOLLOW** (`settings/OptionsSetup.lua`): with `LibKa0s` absent they answer an empty table, so the library-less load registers **112 of the addon's 228** schema rows. §1 requires the stub to be load-completing and its suite to pin the row count against a full load. | The canonical font / border / bar / color-pair / master-controls blocks live **only** in `libs/LibKa0s/OptionsCompose.lua`. A stub that answered them would be a host copy of exactly the row sets the composers were extracted to stop nine addons from re-deciding (options-ui-§16, anti-pattern #73) — and it is the copy that goes stale the day a block grows a row. §1's own list of what must not be copied into this stub (a widget maker, the flow engine, the header, a layout constant) is the same argument one layer up. What the stub still guarantees is what §1's rule is FOR: no page file raises, so no page loses its whole row set. **The blast radius is measured, and it is smaller than "a third of the schema is missing and `list`, `get`, `set`, `reset` break with it, silently" reads.** `LibKa0s-Slash-1.0` lives in the same `libs/LibKa0s/` folder that §1 requires be vendored **whole** (anti-pattern #48), so the load that loses the composers loses the **schema CLI in the same breath**: `settings/Slash.lua:212-230`'s stub answers `set`/`get`/`list`/`reset` with one *"is unavailable"* line each, for a **host-declared** row exactly as for a composed one. There is no state of this addon in which a composed path is addressable-but-missing. Profile defaults are `defaults/Profile.lua`'s, merged by AceDB in `core/Database.lua:64-75`'s `aceDBDefaults`, and are never read off the schema, so a composed setting a player already made keeps being honored. The three things that read `NS.Settings.Schema` at all are the CLI, the panel and `RestoreAllDefaults`' `sessionOnly` walk — respectively absent, absent, and looking for `state.debugConsole`, whose console window is unavailable on this path too (`core/DebugLogSetup.lua:70-72`). Pinned by *"the hollow composers cost the degraded path no CLI reach it otherwise has"* in `tests/test_options_panel.lua`. | 2026-09-02 — **provisional** | The **sign-off itself**: this row is recorded so the decision is visible, not because it has been ratified. It also retires if `LibKa0s-Options-1.0` ships the composers in a file that loads and answers **without** the Options major (as `LibStub` itself does), **or** options-ui-§1 states which of the two rules wins here. `tests/test_options_panel.lua` pins the exact delta by fingerprint, so the day it changes the case says so. |
...
**One row above is PROVISIONAL, and it is the one decision in this pass that is not the addon's to
make.** The `options-ui-§1` hollow-composer row is recorded here so the decision is *visible*, not
because it has been ratified — it sits between two MUSTs of the same section that this pass put into
conflict, and it will be inherited verbatim by the other eight addons, so it wants a yes or a no from
the standard's owner rather than a per-addon call:

* §1 says the degraded stub **MUST** be load-completing, and its stated harm is a short schema taking
  `list`, `get`, `set`, `reset` and the profile defaults down with it, silently.
* §1 also says the stub **MUST NOT** carry a host copy of library content, and options-ui-§16 /
  anti-pattern #73 say the same thing about the composed row sets specifically.

Since the composers moved schema **content** (not just rendering) behind a library call, no stub can
satisfy both. The addon chose the second, and the first's harm is measured rather than assumed: the
schema CLI is not running on that load at all, so nothing that works loses reach (the row cites the
case that pins it). **What is still owed is the ruling**, and the two shapes that would end the
conflict outright are in the row's re-check trigger. Until then this row is a recorded decision under
