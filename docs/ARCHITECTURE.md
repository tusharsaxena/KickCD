...
| `test [on\|off]` | Test mode: placeholder grids and cast bars while locked; a bare `test` toggles. Drives the General → Master controls "Test mode" row |
...
- Test mode is session-only too (`NS.State.testMode`): off after every `/reload`, refused in combat, and ended when combat starts.
