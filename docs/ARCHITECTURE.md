...
- **Writer.** `core/Database.lua` (`NS.Database`) is the intended single writer but is not yet the only one: it owns the empty-container create (`EnsureSpellList`), the seed routine (`BuildSpells`) and the every-list reset verb (`ResetAllSpells`, behind `/kcd spells resetall`, which reuses the seed routine), while add, remove, reorder and the per-spec reset are still written directly in both `settings/Spells.lua` and the `/kcd spells` handlers in `core/KickCD.lua`, and moving them into `Database` is tracked as [#16](https://github.com/tusharsaxena/KickCD/issues/16).
