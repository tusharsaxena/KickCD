...
- **The rung is (b), lock / unlock (`launcher-§2`).** KickCD has no primary window and its preview *is* unlocking, so the left button calls **`NS.ToggleLock`** (`core/KickCD.lua`) — the same seam `/kcd toggle` runs, landing on `Helpers.SetAndRefresh("locked", v)`. The launcher holds no copy of the lock. Right-click always opens the settings panel and is the library's, not this addon's. While the addon is disabled the left click is refused by the **library's** gate (Launcher version 2), fed `isEnabled = NS.MasterEnabled` and `disabledLine = NS.Slash.DisabledLine`, the dispatcher's own line.
- **The status tooltip is the library's (`launcher-§1`, standard v2.66.0; Launcher version 3).** It draws the title and version, `Enabled`, `Locked`, then `Left-click: Unlock frame` / `Lock frame` (or `disabled — /kcd enable` while disabled) and `Right-click: Open settings`, enabled or disabled. The descriptor only answers it: `version` (the TOC's `## Version`, through `NS.Version()`), `isEnabled`, `isLocked` (`db.profile.locked`) and `leftClickLabel` (through `NS.L`), each asked on every show. No `isTestMode` (there is no test mode) and no `onTooltipShow` (no lines of the addon's own; a host title or hint beside the library's is anti-pattern #89).
