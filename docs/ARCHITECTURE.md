...
Each entry's `enabled` and `category` fields are player preferences, not membership: they have no schema row, and the Spells page's checkbox and dropdown and `/kcd spells enable|disable|category` write them directly. Whether they get rows or a register row is an open owner decision, tracked as [#17](https://github.com/tusharsaxena/KickCD/issues/17).
