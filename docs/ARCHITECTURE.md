...
- **Scope.** GLOBAL, so a profile switch does not move a player's buttons and `options-ui-§12`'s *Reset all settings* — a profile reset — cannot un-hide one they deliberately hid. This addon has never stored a minimap table under `profile`, so adoption moved no stored path and `schemaVersion` did not bump.
