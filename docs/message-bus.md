...
| `Ka0s_KickCD_CONFIG_CHANGED` | `settings/Panel.lua Helpers.FireConfigChanged` — the single emitter. Its callers: `Helpers.Set` (every schema-row write, including `/kcd lock|unlock|toggle` and the Focus `units.focus.link` row → `units`); `Helpers.Coalesced` (holds a batch's announcements and sends each section once when it ends — `Helpers.SetRows`, so "Copy styling from Target" sends `icons`, `castbar`, `label` and `units` once each, not once per row); `core/KickCD.lua` (the slash spells commit); `settings/Panel_Render.lua Helpers.ResetIconPosition`; `settings/Spells.lua` (throttled editor commits); `modules/IconGrid.lua OnDragStop` (anchor save → `general`); `modules/Castbar.lua OnDragStop` (anchor save → `castbar`) | `IconGrid`, `Cooldowns`, `Castbar`, `UnitLabel`, `settings/Spells.lua` | `{ section = "general"\|"icons"\|"castbar"\|"label"\|"spells"\|"units" }` |
| `Ka0s_KickCD_PROFILE_CHANGED` | `core/Database.lua`'s file-local `fireProfileChanged` — the single emitter. Its callers: `Database:OnProfileChanged` (AceDB callback for `OnProfileChanged` / `Copied` / `Reset`; `newProfileKey` is the profile active afterwards, which a copy and a reset leave unchanged, never the copy's source) and `Database:ResetAllSpells` (in-place re-seed of every spec, no AceDB swap) | `IconGrid`, `Cooldowns`, `Castbar`, `UnitLabel`, `settings/Spells.lua` | `{ newProfileKey }` |
| `Ka0s_KickCD_GRID_LAYOUT` | `IconGrid:Layout` (every pass, including the empty-list case — one instance per unit, `IconGrid`'s target and focus instance managers each fire their own) | `Castbar`, `UnitLabel` | `{ unit, gridFrame, primaryIcon, width, height }` |
...
Section-keyed for cheap dispatch. `Cooldowns` only acts on `"general"` (master enable) and `"spells"` (rebuild watched list); `IconGrid` acts on `"general"` / `"icons"` / `"spells"` / `"units"`; `Castbar` acts on `"general"` / `"castbar"` / `"units"`; `UnitLabel` re-applies on any section (it has no state of its own beyond what a re-`Apply` recomputes); the Spells panel filters on `"spells"`. The session-only Test mode row (`state.testMode`) announces `"general"` from its own `set()`, so IconGrid and Castbar re-apply visibility and the preview when it flips. The `"label"` section is emitted by every `settings/Label.lua` schema-row write. There is no `"debug"` section — the debug enabled-flag is the session-only `NS.State.debug` (never in SavedVariables), toggled via `DebugLog:SetEnabled` / `/kcd debug on|off|toggle`, and it does **not** broadcast on the bus at all, so flipping it never cascades into a `Cooldowns:Rebuild` / `IconGrid` relayout.
