...
| `Ka0s_KickCD_ConfigChanged` | `settings/Panel.lua Helpers.FireConfigChanged` — the single emitter. Its callers: `Helpers.Set` (every schema-row write, including `/kcd lock|unlock|toggle` and the Focus `units.focus.link` row → `units`); `Helpers.Coalesced` (holds a batch's announcements and sends each section once when it ends — `Helpers.SetRows`, so "Copy styling from Target" sends `icons`, `castbar`, `label` and `units` once each, not once per row); `core/KickCD.lua` (the slash spells commit); `settings/Panel_Render.lua Helpers.ResetIconPosition`; `settings/Spells.lua` (throttled editor commits); `modules/IconGrid.lua OnDragStop` (anchor save → `general`); `modules/Castbar.lua OnDragStop` (anchor save → `castbar`) | `IconGrid`, `Cooldowns`, `Castbar`, `UnitLabel`, `settings/Spells.lua` | `{ section = "general"\|"icons"\|"castbar"\|"label"\|"spells"\|"units" }` |
