...
| `Ka0s_KickCD_CONFIG_CHANGED` | `settings/Panel.lua Helpers.FireConfigChanged` — the single emitter. Its callers: `Helpers.Set` (every schema-row write); `core/KickCD.lua` (lock/unlock, and the slash spells commit); `settings/Panel_Render.lua Helpers.ResetIconPosition`; `settings/General.lua` (the Units tab's focus link toggle / copy-styling button → `units`); `settings/Spells.lua` (throttled editor commits); `modules/IconGrid.lua OnDragStop` (anchor save → `general`); `modules/Castbar.lua OnDragStop` (anchor save → `castbar`) | `IconGrid`, `Cooldowns`, `Castbar`, `UnitLabel`, `settings/Spells.lua` | `{ section = "general"\|"icons"\|"castbar"\|"label"\|"spells"\|"units" }` |
