...
| `Ka0s_KickCD_PROFILE_CHANGED` | `Database:OnProfileChanged` (AceDB callback for `OnProfileChanged` / `Copied` / `Reset`); `Database:ResetAllSpells` (in-place re-seed of every spec, no AceDB swap) | `IconGrid`, `Cooldowns`, `Castbar`, `UnitLabel`, `settings/Spells.lua` | `{ newProfileKey }` |
...
1. The source emitter(s) — the table above is sender-authoritative. Default to a single emitter per message; a second one needs a recorded justification and must live in the same file as the first, so the message still has one owning module. Today only `Ka0s_KickCD_CONFIG_CHANGED` (multi-module by design) and `Ka0s_KickCD_PROFILE_CHANGED` (two call sites, both in `core/Database.lua` — see [ARCHITECTURE.md](ARCHITECTURE.md#invariants-worth-not-breaking)) have more than one.
