...
```
diff -r --strip-trailing-cr <tag>/LibKa0s KickCD/libs/LibKa0s   # 1 file: OptionsCompose.lua
diff -r --strip-trailing-cr <tag>/testkit KickCD/tests/_kit     # empty
```
...
Majors reached from this addon's own source: Core DebugLog Env Media Options Perf Pool Slash Widgets 
