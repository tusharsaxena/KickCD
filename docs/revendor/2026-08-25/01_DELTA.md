# 01 — Delta: KickCD vs LibKa0s v1.15.0
...
```sh
grep -in 'bundles' KickCD/CLAUDE.md
```

> Bundles [LibKa0s](https://github.com/tusharsaxena/LibKa0s) v1.15.0 (MIT). That line is the answer to

## 3b. Actual version — the vendored minors

```sh
grep -hoE 'local +(MAJOR, )?[A-Z_]*MINOR *= *("[^"]+", *)?[0-9]+' KickCD/libs/LibKa0s/*.lua
```
...
```sh
diff -r --strip-trailing-cr <scratch>/LibKa0s KickCD/libs/LibKa0s   # 0 lines
diff -r                     <scratch>/LibKa0s KickCD/libs/LibKa0s   # 0 lines
diff -r --strip-trailing-cr <scratch>/testkit KickCD/tests/_kit     # 0 lines
diff -r                     <scratch>/testkit KickCD/tests/_kit     # 0 lines
```
...
```sh
grep -rnoE 'LibStub\("LibKa0s-[A-Za-z]+-1\.0", true\)' KickCD --include='*.lua' | grep -v '/libs/' | grep -v '/tests/'
```

- `KickCD/core/CoreSetup.lua:64` — Core
- `KickCD/core/DebugLogSetup.lua:41` — DebugLog
- `KickCD/core/EnvSetup.lua:61` — Env
- `KickCD/core/MediaSetup.lua:57` — Media
- `KickCD/core/PerfSetup.lua:33` — Perf
- `KickCD/settings/OptionsSetup.lua:35` — Options
- `KickCD/settings/Slash.lua:55` — Slash

Majors present in the payload with **no lookup anywhere in this addon**: `Item`, `Pool`, `Widgets`.

## 3f. Kit revision and the pairing rule

```sh
grep -n 'Kit.VERSION' <scratch>/testkit/framework.lua KickCD/tests/_kit/framework.lua
```
...
| Gate | Command | Result |
|---|---|---|
| Lint | `luacheck .` | 0 warnings / 0 errors in 35 files |
| Tests | `lua tests/run.lua` | 780 passed, 0 failed, 0 skipped |
