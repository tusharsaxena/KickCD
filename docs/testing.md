...
A headless Lua unit harness lives under `tests/` — run `lua tests/run.lua` from the repo root (exits non-zero on any failure); `luacheck .` must stay at 0 errors. The suites (`test_util`, `test_schema`, `test_database`, `test_bus`, `test_compat`, `test_debuglog`, `test_icongrid_layout`, `test_lifecycle`) load every source under a WoW-API mock and assert pure logic, the message bus, and — via `test_lifecycle` — the full `OnInitialize → OnEnable` cascade that AceAddon fires on `PLAYER_LOGIN`. The harness cannot render frames or model taint, so it complements rather than replaces the in-game checks.
