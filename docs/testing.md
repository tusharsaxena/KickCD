...
- `/kcd test` / `/kcd test on` / `/kcd test off` — test mode: the grids and cast bars show their placeholder view while locked. Routes through `Helpers.SetAndRefresh("state.testMode", ...)`, so the General → "Test mode" checkbox follows. Refused in combat, and ended by `PLAYER_REGEN_DISABLED` with one chat line. Covered headlessly in `test_slash`, `test_state`, `test_schema`, `test_castbar_frame` and `test_icongrid_visibility`.
