...
kit's and the kind that matters most here is a frame: the per-unit `UNIT_SPELLCAST_*` cast-filter
frames. (`core/State.lua`'s combat listener is an AceEvent target, so the kit's half sees it.) Both halves **remove on unregister**,
...
`Castbar.ResolveGridFrame` looks like it belongs in that list but does **not**: `modules/Castbar_Skin.lua` genuinely calls it to resolve the auto-size reference frame, so it is a real cross-file dependency rather than a harness hook. Its comment says so, to keep a future dead-export sweep from mistaking it for one. The same holds for `Castbar.{Instances,Cfg,ForEachEnabled,IsVisible,ApplyVisibilityMask}`, which `modules/Castbar_Events.lua` binds at load because the event and message handlers moved there (#24).
...
agree: `../LibKa0s` sits on **v1.62.0**, [`CLAUDE.md`](../CLAUDE.md) names the same tag, and all
...
That guard is no longer alone: every adopted major now carries the same shape in its own suite — `tests/test_slash.lua`, `tests/test_debuglogsetup.lua`, `tests/test_coresetup.lua` and `tests/test_options_panel.lua` alongside `tests/test_perfsetup.lua`. The Options one is shaped differently on purpose, and the difference is worth copying: `libs/LibKa0s/Options.lua` never reads a descriptor `L` at all, so the trap is not *expressible* for that major today and there is no rendered string to assert on. What it pins instead is that absence, by scanning every file of the major — **five** at LibKa0s v1.39.0: `Options.lua`, `OptionsWidgets.lua`, `OptionsTabs.lua`, `OptionsCompose.lua`, `OptionsScroll.lua` — so a future minor that grows an `L` hook reddens here rather than inheriting the trap silently. The list is **derived from the vendored XML**, not typed (`testing-§9`), and the count is asserted beside it: a typed list does not redden when a file joins the major, it just stops covering it, which is exactly what happened when `OptionsCompose.lua` and then `OptionsTabs.lua` arrived. `OptionsWidgets.lua` is in that sweep because it is where the rendered labels actually come from, which makes it the likeliest of the five to grow one.
...
- `/kcd help` — print the slash command help. Bare `/kcd` runs `config` instead and opens the settings panel on its landing page. `/kickcd` is a long-form alias that routes to the same handler. Every printed line should carry a cyan `[KCD]` prefix (added by `Util.print`); each help row should show the slash invocation in yellow and the description in white.
...
- `/kcd resetposition` — restore every unit's icon grid (target and focus) to its default screen position; a free-moving cast bar keeps its place. Mirrors the General → "Reset position" button.
...
- `/kcd diagnostics` — write the diagnostic report into the debug console (`debug-logging-§14`), appended after the trace, whether logging is on or off and whether the addon is enabled or not; one chat line gives the line count. `/kcd debug diagnostics` is the same report, and there is no `diag` alias. Covered headlessly by `test_diagnostics` (sections, order, budget, read-only, stood down), the kit's `test_diagnostics_contract` run against `/kcd`'s dispatcher, and `test_disabled` (both forms while disabled). The sections are in [debug.md](debug.md#kcd-diagnostics-the-report-debug-logging-14).
