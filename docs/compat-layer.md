...
| `Compat.DebugInterrupt(unit, emit)` | n/a — diagnostic over `_G.UnitCastingInfo` / `_G.UnitChannelInfo` | Backs `/kcd debug interrupt`. `emit` is an optional line sink; without it the lines go to chat through `NS.Util.print`. Prints every positional return with its `type()` and `issecretvalue()` flag, plus what `NS.State.IsHostileUnitCasting` and the visibility/glow logic decided. Renders through a `safeRender` helper that short-circuits secret values to `<secret>`, so the dump never `tostring`s a secret. |
