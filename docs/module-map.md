...
│                       session-only testMode + IsPreviewing)
...
└── settings/
    ...
    ├── Panel.lua     — what is HOST-SHAPED about the canvas framework, all of
                        ...
                        off NS.DebugLog and state.testMode off NS.State
                        rather than off the profile) and the
    ...
    ├── General.lua   — 2 tabs. Master controls (7): the CANONICAL block
                        ...
                        session-only Debug console and Test mode — plus the
                        Reset position / Reset all button pair, which is the
                        composer's own
