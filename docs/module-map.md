...
│                       "KickCD — Debug" (KickCDDebugWindow) built lazily,
...
│                       published as IconGrid.Layout. Pure parsers are unit-
...
11. `modules/DebugLog.lua` — **loads first** of `modules/`. On-screen debug console: a lazily-built DIALOG-strata window "KickCD — Debug" (global frame `KickCDDebugWindow`) with a `ScrollingMessageFrame` log, Copy / Clear buttons, and a header `Debug:ON/OFF` toggle (`DebugLog:SetEnabled` is the single write seam), rendered in the JetBrains Mono font shipped from `media/fonts/` and registered with LibSharedMedia at load. Exposes the sink `NS.Debug(tag, fmt, ...)` — gated on the session-only `NS.State.debug`, routing formatted lines to the console **not** chat — plus the pure `FormatPlain` / `FormatColored` formatters. Inert at file load; every frame is built in `EnsureWindow`.
...
14. `modules/IconGrid_Layout.lua` — icon-grid geometry peeled from IconGrid (KCD-05): anchor/grow token parsing and block placement (`parseAnchor` / `parseGrow` / `placeBlock` / `layoutBlock`), published on the module as `IconGrid.Layout`. The parsers are pure (numbers/strings in and out) and unit-tested headlessly; `layoutBlock` is the frame-manipulating orchestrator that positions the primary + secondary buttons against the grid frame.
