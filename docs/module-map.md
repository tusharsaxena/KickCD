...
│                       unit's chosen attach frame on every Apply
│                       (Castbar:GetCastbarFrame(unit) or
│                       IconGrid:GetGridFrame(unit), per label.style.attach)
│                       so the label inherits that frame's own shown state +
│                       effective alpha — it follows General visibility the
│                       same way the grid/cast bar do. SetPoint-anchored to
│                       the same attach frame for position. Text/show read
│                       via NS.Units.Label (per-unit, never link-resolved);
│                       appearance (incl. label.style.color) read via
...
│                       (further gated live by the parent's own visibility).
...
└── settings/
    ├── Panel.lua     — top-level category + canvas-panel framework + schema
                        ...
                        alwaysPerUnit flag; the flag/mechanism remains generic
                        infra, but as of the Label defaults follow-up no
                        schema row currently sets it — Label's show/text
                        rows dropped it, so every panel's PartitionUnitRows
                        call currently yields an empty perUnit set)
...
- **The `alwaysPerUnit` schema-row flag** (`settings/Panel.lua`'s `Helpers.PartitionUnitRows`) — added so a linked Focus's appearance rows could stay hidden/mirrored while its label `show`/`text` (per-unit identity data, like `label.text` always was) stayed editable. A follow-up dropped `alwaysPerUnit` from the two Label rows so a linked Focus's Text Label page collapses to just the "Linked to Target" note, matching Icons/Cast bar; `show`/`text` are still per-unit in the DB, just not editable on the page while linked. The flag/mechanism remains as generic infra in `PartitionUnitRows` for any future row that needs the exception — no schema row currently sets it.
