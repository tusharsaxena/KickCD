...
[`20260908-181321/test-cases.md`](20260908-181321/test-cases.md) is the authority on which cases existed at this run;
`docs/test-cases.md` is that same list at HEAD.

Moved **780 → 860** since the previous run.
...
[`20260908-181321/perf.json`](20260908-181321/perf.json).
...
Current as of [`20260908-181321`](20260908-181321/) — **this run's measurement, not its diff.** Max CCN **15** across 2268
...
| 1000–1500 (on notice) | `modules/IconGrid.lua` | 1152 | **Already tracked as `A-2`.** Effectively flat — 1153 at the previous run's commit, one line down. The layout pass already lives in a sibling file, `modules/IconGrid_Layout.lua`, so the peel this file would take has been taken. Watch, no action. |
...
| 1000–1500 (on notice) | `tests/wow_mock.lua` | 1232 | **Already tracked as `KCD-30`.** 1128 at the previous run's commit, +104 this cycle as the mock grew to cover what the new cases exercise. Not covered by `A-2`, which lists source files only; the whole file is the deviation, and the tracked fix rebuilds the mock as a thin extender rather than trimming it. It is the largest file under `tests/` and, since `M4-11`, `luacheck` does see it — the lint scope went 35 files to 93. |
