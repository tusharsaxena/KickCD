...
Slash commands that mutate schema-backed fields (e.g. `/kcd lock`) route through `Helpers.SetAndRefresh(path, value)` so they share the panel widgets' write/notify/refresh code path (`Helpers.Set` → schema row's `onChange` → `Helpers.RefreshScalars` — a scalar re-sync, never the structural `RefreshAllPanels`, which would rebuild the page under a widget still being dragged). That way a future `onChange` added to a row doesn't silently diverge between the two paths.
...
- Settings → General → "Lock frame" checkbox writes `db.profile.locked` through `Helpers.Set` and fires `Ka0s_KickCD_ConfigChanged { section = "general" }`.
