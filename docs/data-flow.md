...
The GCD-vs-real-CD visual decision is made entirely C-side via `cdObject:EvaluateTotalDuration(curve)` (`evaluateByTotal`). The IconGrid maintains step-shaped alpha and color curves **per unit**, built from that unit's resolved `cfg.readyAlpha` / `cfg.cooldownAlpha` / `cfg.cooldownTint` (so an unlinked focus honors its own values rather than inheriting target's): a **total** ≤ ~1.6s evaluates to ready visuals, anything beyond evaluates to cooldown visuals. Lua never compares the secret timings directly. The curves read the cooldown's total length rather than its remaining time because the two are indistinguishable near the end of a real cooldown — reading `remaining` made the last ~1.5s of a 15s interrupt cooldown render as a GCD, so the icon showed ready before the spell was. `BuildCurves` skips a unit whose three curve-shaping values haven't moved, so an unrelated `icons` edit (border, font, layout, glow) doesn't recreate them. See `modules/IconGrid_Render.lua` `BuildCurves` / `curvesFor`.

A `Cooldowns:Refresh` whose `PollSpell(id)` returns `nil` for a previously-watched id (pet dismissed, talent untrained, encounter mechanic suppresses the spell) emits a final sentinel `Ka0s_KickCD_SpellState { spellID, ready=false, isActive=false, cdObject=nil, chargeCdObject=nil, charges=nil }` and unwatches the id. Without it the icon would render the last-known state until the next `Rebuild` — which `SPELLS_CHANGED` / `TRAIT_CONFIG_UPDATED` typically cover, but not all paths fire those events.
...
Most user input (settings panel widget, slash `/kcd set`, slash `/kcd lock|unlock|toggle`) flows through the schema seam, `NS.Settings.Store.Set(path, value)` (`LibKa0s-Schema-1.0`, built in `settings/SchemaSetup.lua`), which refuses a path no row declares, writes `db.profile.<path>`, runs the row's `onChange` and fires `Ka0s_KickCD_ConfigChanged { section = row.section }`. IconGrid and Castbar handle each section appropriately. AceDB callbacks fire `Ka0s_KickCD_ProfileChanged` on profile change / copy / reset.

The debug enabled-flag is outside this path: `/kcd debug on|off|toggle` (or the console header's toggle) sets the session-only `NS.State.debug` through `DebugLog:SetEnabled(on)` (default off, never in SavedVariables, resets each `/reload`). Continuous `NS.Debug(tag, fmt, ...)` output routes to the on-screen debug console (`LibKa0s-DebugLog-1.0`, wired in `core/DebugLogSetup.lua`), not the chat frame.

Slash commands that mutate schema-backed fields (e.g. `/kcd lock`) route through `Helpers.SetAndRefresh(path, value)` so they share the panel widgets' write/notify/refresh code path (`Store.Set` → schema row's `onChange` → the announce → `Helpers.RefreshScalars` — a scalar re-sync, never the structural `RefreshAllPanels`, which would rebuild the page under a widget still being dragged). That way a future `onChange` added to a row doesn't silently diverge between the two paths.

Drag is the one exception: each instance's `IconGrid:onDragStop` and `Castbar:onDragStop` save `db.profile.units.<unit>.anchors.icons` / `.castbar` through `NS.Units.SetAnchor(unit, which, Util.SaveAnchor(frame))` rather than the schema-row helper (no row addresses an anchor: it is named non-setting state owned by `Units`, whose writers [ARCHITECTURE.md](ARCHITECTURE.md#settings-schema) lists; position is also never link-resolved, so the write always targets the dragged instance's own unit), then fire `Ka0s_KickCD_ConfigChanged { section = "general" }` and `{ section = "castbar" }` respectively so any future anchor-aware subscriber gets notified. The IconGrid / Castbar own re-anchor handlers are idempotent on the just-saved value, so the re-entrant dispatch is safe. The Reset position button + `/kcd resetposition` go through `Helpers.ResetIconPosition`, which writes every unit's `units.<unit>.anchors.icons` directly from `NS.DEFAULT_PROFILE` and fires the same `general`-section message once (cast-bar anchors are left alone; see [settings-panel.md](settings-panel.md#reset-helpers)).
...
- Settings → General → "Lock frame" checkbox writes `db.profile.locked` through `Store.Set` and fires `Ka0s_KickCD_ConfigChanged { section = "general" }`.
- Slash commands `/kcd lock | unlock | toggle` route through `Helpers.SetAndRefresh("locked", ...)` so they share the same write/notify/refresh path as the checkbox (and so an open General panel reflects the lock state immediately). When that path cannot take the write (the settings layer has not loaded, or a LibKa0s-less load has no composed `locked` row), they print "Settings layer not ready yet" and write nothing. There is no direct-write fallback.
