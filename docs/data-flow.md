...
The GCD-vs-real-CD visual decision is made entirely C-side via `cdObject:EvaluateRemainingDuration(curve)`. The IconGrid maintains step-shaped alpha and color curves **per unit**, built from that unit's resolved `cfg.readyAlpha` / `cfg.cooldownAlpha` / `cfg.cooldownTint` (so an unlinked focus honours its own values rather than inheriting target's): remaining ≤ ~1.6s evaluates to ready visuals, anything beyond evaluates to cooldown visuals. Lua never compares the secret remaining time directly. `BuildCurves` skips a unit whose three curve-shaping values haven't moved, so an unrelated `icons` edit (border, font, layout, glow) doesn't recreate them. See `modules/IconGrid_Render.lua` `BuildCurves` / `curvesFor`.
