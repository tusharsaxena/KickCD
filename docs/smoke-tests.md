...
- **Resets.** Move two Cast bar settings off their defaults, then press the Cast bar page's **Defaults**: one `[Set] reset castbar: 2 rows` line, and no per-row `[Set] units.…` line. Press **Defaults** again: `[Set] reset castbar: 0 rows`. Then **Reset all settings** (or `/kcd resetall`): exactly **one** `[Set]` line, `[Set] reset profile '<name>' to defaults (N rows)`, with no `[Set] reset all` line beside it and no `[Profile] switched` line. A single `/kcd set locked true` afterwards logs its own `[Set] locked = true`, so the mute did not stick.
