...
- **Test mode** (`preview-mode`, `options-ui-§15`). The General → Master controls "Test mode" checkbox, and `/kcd test [on|off]`, show every enabled grid and cast bar with placeholder content while they stay locked. Session-only, refused in combat, and ended when combat starts. Unlocking still shows the same preview while you drag. The original TestMode module was removed at commit `59fb5c0` and this was out of scope until standard v2.47.0 made a test mode required for every positionable display.
...
- `/kcd spells reset [CLASS SPEC]` — restores a single spec list to defaults, plus the player's racial cast-stopper when it is their own class; leaves every other spec untouched. The settings panel's per-spec "Defaults" button (the `KICKCD_RESET_SPELLS` popup) is the GUI entry point for the same recovery.
