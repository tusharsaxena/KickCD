...
- **Target + focus dual tracking.** Focus is on by default and links to target's icon grid + cast bar appearance (`units.focus.link`, default on) — or can be styled independently via "Copy styling from Target" + manual edits. Position and the optional per-unit identity label stay independent regardless of link state. One shared drag lock and one shared visibility mode still cover both units — see [ARCHITECTURE.md](ARCHITECTURE.md#invariants-worth-not-breaking).
