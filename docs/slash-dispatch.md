...
| `lock` / `unlock` / `toggle` | Set / clear / flip `db.profile.locked`. | Routes through `Helpers.SetAndRefresh("locked", ...)` so the General → "Lock frame" checkbox refreshes and any future onChange wired onto the schema row fires. When that path cannot take the write (the settings layer is not loaded, or a LibKa0s-less load composes no `locked` row) it prints "Settings layer not ready yet" and writes nothing; there is no direct-write fallback. |
| `test [on\|off]` | Turn test mode on or off; a bare `test` toggles. | Routes through `Helpers.SetAndRefresh("state.testMode", ...)`, the General → Master controls "Test mode" row, so the checkbox follows. Session-only (`NS.State.testMode`). A start in combat is refused by the row's `set()`, which prints why; with no row it prints "Settings layer not ready yet", as `lock` does. |
