...
| **Reset all settings** and `/kcd resetall`, live or with LibKa0s absent | `[Set] reset profile '<name>' to defaults (N rows)` alone, from `Database:OnProfileChanged` ([profiles.md](profiles.md#reacting-to-a-profile-change)). The bracket sees `info.profileReset` and adds nothing; the degraded stub runs its walk under `MuteSetLog` |
