...
* **`Minimap button` inverts, and stores in the GLOBAL tree.** The row is the composer's (`minimapPath`, compose minor 7) and its path is `global.minimap.shown`, taken verbatim — it addresses **LibDBIcon-1.0's own table**, `db.global.minimap`, which is outside the block's profile prefix. The path reads in the row's own sense (`launcher-§3`, standard v2.65.0) while the stored boolean is LibDBIcon's `hide`, which says *hidden* — no `shown` key is ever written — so the row's get/set **invert** in `settings/Panel.lua`'s `GLOBAL_PATHS`, beside `SESSION_PATHS` and inside the same `Helpers.Get`/`Set` — the addon's single write seam (`options-ui-§1`). The `set` also calls `NS.Launcher:SetShown`, so the button follows the checkbox immediately rather than at the next reload.
  ...
  **No reset may touch it, and `launcher-§3` (v2.54.0) states that as a property of the setting** rather than deriving it from the store: whether the button is shown is a per-installation display preference, like the position LibDBIcon keeps in the same table. *Reset all settings* never reached it here — it is a profile reset and the table is global, and the walk narrows to `sessionOnly` rows besides — but **the General page's *Defaults* button did**, because `RestoreDefaults` walks every row on the page and reads no veto. `settings/OptionsSetup.lua`'s `survivesEveryReset` returns early inside the descriptor's `applyDefault`, the one funnel both resets write through. The object behind it is `core/LauncherSetup.lua`; see [ARCHITECTURE.md](ARCHITECTURE.md).
...
The **"Use same styling as Target"** checkbox (the `units.focus.link` schema row, declared `skipRender` so the library does not draw a second checkbox, and written through `Helpers.SetAndRefresh`) and the **"Copy styling from Target"** button (`NS.Units.CopyStyling("target", "focus")`, a one-time copy that also flips `link = false`) live on **General → Units** now (`settings/General.lua`, the group's `afterGroup`), **paired on one line** — `[tick][button]` — through `Helpers.RenderGrid`, the flow engine's caller-driven sibling. They were two lines: a half-width tick, then an `InlineButtonPair` holding a single button, which reads as a button belonging to whatever follows it rather than to the tick above. They are two halves of one decision — does Focus follow Target, and if not, start it from Target's current look — so they pair. The button is built by `copyStylingButton` into the cell the grid hands it, rather than by `InlineButtonPair`, which owns a full-width row of its own and so can never share a line. They used to be drawn three times over, once in each unit page's hand-built header; there is exactly one of them — whether Focus keeps its own appearance or mirrors Target's — and the scroll, the only place left to draw them on a tabbed page, is cleared out from under them by every tab click. Both end in `Ka0s_KickCD_ConfigChanged{section="units"}` (from `Helpers.Set`, through the link row's section) and one `Helpers.RefreshAllPanels`, which is a **structural** refresh: it re-renders every page that declared a renderer and marks the hidden ones dirty so they repaint on their next `OnShow`. That is what lets the control reach the three unit pages from another page at all. The refresh is the link row's `onChange`, and it fires **only when the link moves**: `writeRow` hands every `onChange` the value it replaced as a second argument, so a write that leaves the link where it was (a Defaults on an already-linked Focus, a repeated `/kcd set`) rebuilds nothing. When the page's own Defaults does flip it, that refresh re-renders General **inside** the library's `RestoreDefaults` row loop, and the page comes back whole (`tests/test_units.lua` pins both).

**The copy is a walk of rows, through the helper** (`architecture-§5`: a copy-from that touches rows is a helper write). `CopyStyling` reads every row under the source unit's `icons.`, `castbar.` and `label.style.`, plus `label.show`, and hands the lot, with `link = false` last, to `Helpers.SetRows`. That function writes each row the way `SetAndRefresh` does (`Helpers.Set`, then the row's `onChange`) but inside `Helpers.Coalesced`, so the bus hears each section once after the last row rather than once per row. The debug log is coalesced the same way: `CopyStyling` hands `SetRows` a summary, so each row's `[Set]` line is muted (`Helpers.MuteSetLog`) and the console shows one `[Set] copy target→focus: N rows` instead of about 110 lines (the owner's call; `debug-logging-§10`). N is the rows whose value the copy changed, not the rows it walked: a Focus row already equal to Target's is not counted, and the link counts when the copy flips it. Only the log is muted, and every row still validates and runs its `onChange`. There is no scalar sweep per row, and there is exactly one structural refresh. When the copy flips the link, that refresh is the link row's own `onChange`, which runs last. When Focus is already unlinked, the link does not move, so that `onChange` stays quiet and `CopyStyling` repaints by itself, once. Schema order matters: `castbar.orientation`'s `onChange` resets `growDirection` to that axis's default, and `growDirection` is declared after it, so the copied value lands last and survives. `label.text` stays per-unit and is never copied. Every leaf under the copied subtrees is a schema row, so walking rows loses nothing a whole-table copy carried, and `tests/test_units.lua` pins that.
...
A bulk copy or reset is logged as **one** `[Set] <act> <scope>: N rows` line, never one `[Set]` per row (`debug-logging-§10`). LibKa0s v1.32.0 brackets its reset walks for this (Options minor 16 around `RestoreDefaults` and `RestoreAllDefaults`, Slash minor 8 around `CliResetAll`). Both descriptors hand the bracket to `Helpers.BulkBegin` / `Helpers.BulkEnd` in `settings/Panel.lua`, which share one record with `Helpers.MuteSetLog`:

- **While an act is open, `logSet` is quiet and `Helpers.Set` records each row's value from before the act's first write to it.** N is the rows whose value differs from that record when the act closes. A row already at its default is not counted, and a row an `onChange` rewrites mid-act counts once. The library's own `count` argument is every row `applyDefault` returned, so the line does not carry it. Validation and each row's `onChange` still run per row.
- **Acts nest and log once.** A depth counter keeps one record across the levels, and only the outermost level logs, with every level's rows. A level that reset the whole profile silences the act.
- **The line is kept at N = 0.** A Defaults press on a page already at its defaults logs `[Set] reset castbar: 0 rows`.
- **An act that ends with an error still logs its one line, marked.** The line counts what was written before the raise and ends ` (stopped by an error)`, e.g. `[Set] reset general: 0 rows (stopped by an error)`. The mute is released, and then the error is re-raised unchanged. This holds for the library's bracket, for `Helpers.SetRows` (whose line `MuteSetLog` now emits itself), and for the degraded Reset all.
- **A write still pending when an act opens is logged first.** A per-row `[Set]` line is debounced, and an act's line is not, so a write made just before a Defaults press used to print after the act's line, carrying a value the act had replaced. Opening an act now flushes the pending lines, in write order, and retires their timers.
...
| **Reset all settings** and `/kcd resetall`, live or with LibKa0s absent | `[Set] reset profile '<name>' to defaults (N rows)` alone, from `Database:OnProfileChanged` ([profiles.md](profiles.md#reacting-to-a-profile-change)). N is the profile rows not at their default just before the reset, counted by `Helpers.ResetProfileCounted`; a profile already at its defaults logs `(0 rows)`. The bracket sees `info.profileReset` and adds nothing. The degraded stub drives `Helpers.BulkBegin` / `Helpers.BulkEnd` itself and sets `profileReset` only once `db:ResetProfile()` returns, so a stub with no db, or a reset that raised, logs `[Set] reset all: N rows` (marked when it raised) instead of nothing |
...
| **Copy styling from Target** | `[Set] copy target→focus: N rows`, from `Helpers.SetRows` |
...
```lua
{
    ...
    onChange = function(v, old) ... end,     -- optional side-effect hook; `old` is the
...
| `Defaults` button | `Helpers.RestoreDefaults(panelKey, ctx)` resets every panel row to `def.default`, runs `onChange`, fires per-section `Ka0s_KickCD_ConfigChanged`, re-runs the panel's refreshers |
