...
* **The anchors and the Focus `link` flag come back for free**, since both live in the profile, so `Helpers.ResetAllPositions` is no longer on this path. It is still defined in `settings/Panel_Render.lua` and still covered by `tests/test_settings_log.lua`, but nothing in the addon calls it any more. Its sibling `Helpers.RestoreUnitLinks` is gone: `units.focus.link` is a schema row, so the General page's **Defaults** and `/kcd reset units.focus.link` restore it too.
