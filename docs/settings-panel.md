...
* **The anchors and the per-unit `link` flag come back for free** — both live in the profile, so `Helpers.ResetAllPositions` and `Helpers.RestoreUnitLinks` are no longer on this path. They are still defined in `settings/Panel_Render.lua` and still covered by `tests/test_settings_log.lua`, but nothing in the addon calls either any more.
