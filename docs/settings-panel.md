...
Four pages sit under **Ka0s KickCD** in the game's own settings. This is the page-granularity view; the tab strip below is the same thing one level finer.
...
On the Grid page, the Unit picker above the rail and the tabs scopes every entry. The focus set starts **linked** to target, copying its icon and cast-bar styling; General → Units is where that link is broken. Position and label text stay per-unit either way.
...
A page with exactly **one** section draws a **one-tab** strip. That is the library's behavior as of `OptionsWidgets` minor 13 — the `#groups < 2` fallback to `RenderSchema` is gone — and it is why the Spells page, which has no schema rows at all, draws its single tab by hand.
...
| **Grid → Icons** | Sizing (4) \| Layout (6) \| Visual states (5) \| Border (5) \| Annotations (11) \| Ready glow (8) | 39 per unit |
| **Grid → Cast bar** | General (5) \| Size and position (7) \| Icon (2) \| Font (6) \| Spell name (5) \| Cast time (4) \| Interruptible (13) \| Non-interruptible (13) | 55 per unit |
| **Grid → Text Label** | General (2) \| Placement (8) \| Font (6) | 16 per unit |
...
There are no hand-rolled headings anywhere — a colored full-width `Label` standing in for a `Heading` is anti-pattern #71, and `tests/test_schema.lua` scans `settings/` for the gold it would be written in.
...
* **`state.debugConsole` resolves outside the profile.** It is the composer's session-only row, and its path names a live object rather than a saved key, so `settings/General.lua` wires a `get`/`set` pair onto the row that answers it off `NS.DebugLog`, and the schema seam (`Store.Get` / `Store.Set`) calls that pair instead of touching the profile. `debug-logging-§5` still holds — nothing about the console ever reaches SavedVariables — and `/kcd get|set|list state.debugConsole` now works, which the bespoke `SessionToggle` it replaced never allowed.
* **No `Test mode` row.** `options-ui-§15` (v2.49.0) exempts an addon whose unlocked view already shows its placeholder preview, and KickCD's does: unlocked, the grids ignore the visibility mode and the cast bar shows its placeholder. *Lock frame* is the switch, so the spec passes no `testModePath` and there is no `/kcd test` verb. It is also why the minimap button's hover tooltip (`launcher-§1`, drawn by `LibKa0s-Launcher-1.0`) reports `Enabled` and `Locked` and has no `Test mode` line, why its right-click options menu (`launcher-§2`, standard v2.67.0) has exactly two entries, *Enabled* (`NS.SetMasterEnabled`, the handler `/kcd enable` / `disable` run) and *Locked* (`NS.ToggleLock`, the handler `/kcd toggle` runs), and why **Minimap button** sits alone on its line: the composer drops that row's `startsLine` only when a test mode is there to pair beside it.
* **`Minimap button` inverts, and stores in the GLOBAL tree.** The row is the composer's (`minimapPath`, compose minor 7) and its path is `global.minimap.shown`, taken verbatim — it addresses **LibDBIcon-1.0's own table**, `db.global.minimap`, which is outside the block's profile prefix. The path reads in the row's own sense (`launcher-§3`, standard v2.65.0) while the stored boolean is LibDBIcon's `hide`, which says *hidden* — no `shown` key is ever written — so the row's own get/set, wired onto it in `settings/General.lua`, **invert** it — and the schema seam calls that pair, so the row still goes through the addon's single write seam (`options-ui-§1`). The `set` also calls `NS.Launcher:SetShown`, so the button follows the checkbox immediately rather than at the next reload.

  There is **one** boolean, and LibDBIcon writes it too (its own `Hide` / `Show`, which a broker display's hide toggle calls); a second `minimap.show` beside it would be a copy free to disagree (anti-pattern #81). The **global** scope is `launcher-§3`'s decision rather than an accident: a minimap button belongs to the installation, so a profile switch must not move a player's buttons. The row is **stored**, never `sessionOnly`, which is what keeps `vetoedFromResetAll` off it.

  **No reset may touch it, and `launcher-§3` (v2.54.0) states that as a property of the setting** rather than deriving it from the store: whether the button is shown is a per-installation display preference, like the position LibDBIcon keeps in the same table. *Reset all settings* never reached it here — it is a profile reset and the table is global, and the walk narrows to `sessionOnly` rows besides — but **the General page's *Defaults* button did**, because `RestoreDefaults` walks every row on the page and reads no veto. The schema seam's `resetExempt` (`settings/SchemaSetup.lua`) now refuses the row inside `Store.ApplyDefault` **while a bulk bracket is open**, and both resets write through `applyDefault` inside the bracket the Options descriptor hands them, so the one row is exempt from both from one place while `/kcd reset global.minimap.shown` — one row named out loud, outside any bracket — still resets it. The object behind it is `core/LauncherSetup.lua`; see [ARCHITECTURE.md](ARCHITECTURE.md).
...
The cast bar's new color governs the **cast time** text, and is labeled *Cast time color* for that reason. The spell name keeps its two per-state `nameTextColor` swatches: the choice between them is a curve evaluation on a possibly-secret flag, so a third writer over the same `FontString` would simply lose to them.

## The class-color companion (`options-ui-§17`)

**Every color picker carries a `Use class color` checkbox immediately to its right**, and the composers set `startsLine` on the swatch so an odd number of widgets above the pair can never split it across two lines. Thirty swatches, fifteen per unit, every one of them paired.

Resolution goes through **one** resolver — `NS.ResolveColor` (`core/CoreSetup.lua`, handed over from `LibKa0s-Core-1.0`) — which owns the three rules a host would otherwise re-decide per surface: the stored **alpha** always applies, an **unresolvable class falls through to the stored swatch** (never to white, never to a substitute hue), and the swatch is therefore **never disabled**. `disabledIf` on a color row is forbidden (anti-pattern #74) and the rule is said in the swatch's tooltip instead.

**Which class — and the path does not decide it:**

| Surface | Source | Why |
|---|---|---|
| Icons: cooldown tint, icon border, cooldown-text color, both glow colors | **player** | the grid counts down the PLAYER'S OWN interrupts — `modules/Cooldowns.lua`'s `ResolveClassSpec` and `modules/IconGrid.lua`'s `getActiveSpecKey` both key the watched list on `UnitClass("player")`. That these settings live under `units.<unit>.` says nothing about whose class they mean. |
| Cast bar: both bar colors, both backgrounds, both spell names, both borders, the cast-time color | **unit** | the bar describes the tracked unit |
| Text Label: the label color | **unit** | the label names the unit it is drawn beside |
...
## The Grid page's entries (Icons / Cast bar / Text Label)

These three render through `Helpers.RenderGridPage`, which hands each to `Helpers.RenderUnitPanel(ctx, panelKey, afterGroup, chrome)` (`settings/Panel_Render.lua`). That call does three things in this order, and the order matters. The Grid page's rail goes in through `chrome`, between the first and the other two:
...
1. **The Unit picker, as the page banner** (`Helpers.PageBanner`, `options-ui-§14`). It is pinned in the page's **chrome band, above the tab strip** — not added to the scroll, and not a tab. It scopes the whole page: `ctx.unit` is stashed on the panel ctx so `Helpers.SchemaForPanel(panelKey, ctx.unit)` filters rows to the selected unit's `units.<unit>.icons.*` / `.castbar.*` / `.label.*` paths, and every tab on the page edits that unit.
   ...
   **The selection is shared by all three entries, and it is session-only.** `Helpers.ViewedUnit` / `Helpers.SetViewedUnit` read and write one value on `NS.State` (`core/State.lua`), and `RenderUnitPanel` reads it rather than defaulting from the ctx. It was per-ctx, so picking Focus on Icons and walking to Cast bar arrived back on Target — the reader re-picked the unit on each page and nothing on screen said why it had moved. The picker names which half of the addon you are configuring, which is a property of the reader's session and not of the page they happen to have open. It is deliberately **not** a SavedVariable: where someone is looking is not something they configured, and persisting it would open a fresh login on whichever unit was selected weeks ago with nothing explaining why. Selecting a unit publishes a **structural** `Helpers.RefreshAllPanels`: the Grid page re-renders on the new unit and keeps its entry and tab, and a hidden page is marked dirty so it repaints on its next `OnShow`.
...
2. **The linked-Focus branch** (`Helpers.RenderLinkedUnit`). When `ctx.unit` is a linked Focus, the page draws **the strip first, always**, then the `alwaysPerUnit` rows (there are none today) and a note reading *"Linked to Target. Untick 'Use same styling as Target' on the General page's Units tab to give Focus its own."*
   ...
   **That strip is inert**, and the note is a **link**. Two things follow from there being no `alwaysPerUnit` row: every tab on a linked page draws the same thing, so a strip you can click redraws the identical page — a control that appears to do something and does not, which is the same defect the ↑/↓ arrows had before the reorder lists took a drag. `Helpers.RenderLinkedUnit` therefore runs `deadenStrip` over the buttons `TabStrip` returns: every one disabled, every one of their textures desaturated so it reads as deliberate rather than broken. The strip still **draws** (`options-ui-§13` — the page must not change shape when the picker flips), it just cannot be operated. Done on the returned buttons rather than through a flag on the tab spec because the spec has none and this is one page in one addon; if a second page ever needs it, that is when it becomes `disabled` on the spec and moves into LibKa0s.

   The note is an `InteractiveLabel` drawn by `Helpers.LinkRow`, with the phrase naming the destination colored in Blizzard's link blue (`Helpers.LinkText`) and the click wired to `Helpers.OpenPageTab("general", L["Units"])`. It sets **no hover highlight**: AceGUI forwards `SetHighlight` to `Texture:SetTexture`, whose four-number form is the deprecated color API, and the client answered `(1, 1, 1, 0.12)` with a solid bright-green block across the whole line on mouseover. The link color is what marks the line as clickable, so nothing is lost by drawing no plate behind it. It named a control and the page holding it and then left the reader to find both by hand, two categories away in Blizzard's list. The whole line takes the click — AceGUI has no widget that mixes clickable and static runs inside one string — and the color on the middle phrase is what says so. `OpenPageTab` sets the destination page's `activeTab` **before** the category switch, so General opens already on Units rather than jumping a frame later, and it **refuses under combat** for the reason `/kcd config` does: Blizzard's category switch is protected, and calling it under lockdown taints the panel for the session (`options-ui-§2`). The page's Blizzard category comes from `NS.Settings.categoryFor`, which `settings/OptionsSetup.lua` fills by wrapping each builder — the library's registry drops the builder's return value, so the id is otherwise unrecoverable.
   ...
   **The library's `RenderTabbedSchema` opts were evaluated for this page and declined** (KC-20, [issue #23](https://github.com/tusharsaxena/KickCD/issues/23), closed as `state:will-not-do`). Its `disabledFor` draws `disabledNotice` **above** the rows and still draws every row disabled, where this page needs the note **in place of** them; the notice is a plain `TextRow`, not a link; and the library leaves the strip operable, reachable from `chrome(ctx)` only through the private `ctx.__tabLayout` (LibKa0s `docs/api/Options/version-24.31.4.7.4-docs.md`). `tests/test_options_panel.lua` pins the page — full strip, inert, the link note alone — so a later adoption must keep that case green unchanged.

3. **The strip and the rows**, via `Helpers.RenderTabbedSchema`.

The **"Use same styling as Target"** checkbox (the `units.focus.link` schema row, declared `skipRender` so the library does not draw a second checkbox, and written through `Helpers.SetAndRefresh`) and the **"Copy styling from Target"** button (`NS.Units.CopyStyling("target", "focus")`, a one-time copy that also flips `link = false`) live on **General → Units** now (`settings/General.lua`, the group's `afterGroup`), **paired on one line** — `[tick][button]` — through `Helpers.RenderGrid`, the flow engine's caller-driven sibling. They were two lines: a half-width tick, then an `InlineButtonPair` holding a single button, which reads as a button belonging to whatever follows it rather than to the tick above. They are two halves of one decision — does Focus follow Target, and if not, start it from Target's current look — so they pair. The button is built by `copyStylingButton` into the cell the grid hands it, rather than by `InlineButtonPair`, which owns a full-width row of its own and so can never share a line. They used to be drawn three times over, once in each unit page's hand-built header; there is exactly one of them — whether Focus keeps its own appearance or mirrors Target's — and the scroll, the only place left to draw them on a tabbed page, is cleared out from under them by every tab click. Both end in `Ka0s_KickCD_ConfigChanged{section="units"}` (from the schema seam's announce, through the link row's section) and one `Helpers.RefreshAllPanels`, which is a **structural** refresh: it re-renders every page that declared a renderer and marks the hidden ones dirty so they repaint on their next `OnShow`. That is what lets the control reach the Grid page from another page at all. The refresh is the link row's `onChange`, and it fires **only when the link moves**. The seam hands `onChange` the new value and nothing else, so the row stores through its own `set`, which notes whether the stored link moved before it writes, and `onChange` reads that note — so a write that leaves the link where it was (a Defaults on an already-linked Focus, a repeated `/kcd set`) rebuilds nothing. When the page's own Defaults does flip it, that refresh re-renders General **inside** the library's `RestoreDefaults` row loop, and the page comes back whole (`tests/test_units.lua` pins both).

**The copy is one batch through the seam** (`architecture-§5`: a copy-from that touches rows is a helper write). `CopyStyling` reads every row under the source unit's `icons.`, `castbar.` and `label.style.`, plus `label.show`, and hands the lot, with `link = false` last, to **`Store.SetMany(entries, { act = "copy", scope = "target→focus" })`** — `LibKa0s-Schema-1.0`'s all-or-nothing batch. Every entry is checked first, and one refusal stores nothing. Then every row is stored, then every row's `onChange` runs in order, and the seam's `announceBatch` (`settings/SchemaSetup.lua`) sends each section the batch touched **once**, rather than once per row. `act` makes the batch one bulk bracket, so the console shows one `[Set] copy target→focus: N rows` instead of about 110 lines (the owner's call; `debug-logging-§10`). N is the rows whose read-back value the copy changed, not the rows it walked: a Focus row already equal to Target's is not counted, and the link counts when the copy flips it. There is no scalar sweep per row, and there is exactly one structural refresh. When the copy flips the link, that refresh is the link row's own `onChange`, which runs last. When Focus is already unlinked, the link does not move, so that `onChange` stays quiet and `CopyStyling` repaints by itself, once. **Stores come before reactions**, which is why `castbar.orientation`'s `onChange` resets `growDirection` only when the stored direction is off the new axis: by the time it runs, the copied `growDirection` is already stored, on the axis, and kept. `label.text` stays per-unit and is never copied. Every leaf under the copied subtrees is a schema row, so walking rows loses nothing a whole-table copy carried, and `tests/test_units.lua` pins that.

General has no unit picker — its only per-unit rows are the two `units.<unit>.enabled` toggles on its Units tab, which render together, tagged with `unit = "target"|"focus"` purely so `RestoreDefaults`/`RestoreAllDefaults` reset both units together. (`label.show` / `label.text` used to live here; they moved to the Grid page's Text Label entry, which does use the picker.)

### Bulk resets log one line

A bulk copy or reset is logged as **one** `[Set] <act> <scope>: N rows` line, never one `[Set]` per row (`debug-logging-§10`). LibKa0s v1.32.0 brackets its reset walks for this (Options minor 16 around `RestoreDefaults` and `RestoreAllDefaults`, Slash minor 8 around `CliResetAll`). Both descriptors hand the bracket to the schema seam's own pair, `Store.BulkBegin` / `Store.BulkEnd` (`LibKa0s-Schema-1.0`), and `Store.SetMany` with an `act` opens the same bracket for a batch:

- **While an act is open, the seam logs no per-write line and tallies each write whose read-back value moved.** A row already at its default is not counted. The library's own `count` argument is every row `applyDefault` returned, so the line does not carry it. Validation and each row's `onChange` still run per row.
- **Acts nest and log once.** One tally is kept across the levels, and only the outermost close logs, with every level's rows. A level that reset the whole profile silences the act.
- **The line is kept at N = 0.** A Defaults press on a page already at its defaults logs `[Set] reset castbar: 0 rows`.
- **An act that ends with an error still logs its one line, marked.** The line counts what was written before the raise and ends ` (stopped by an error)`, e.g. `[Set] reset general: 0 rows (stopped by an error)`. The bracket closes, and then the error is re-raised unchanged. This holds for the library's reset walks and for a `SetMany` whose `onChange` raised (every entry was stored before the reactions ran, so they all count).
- **A write still pending when an act opens is logged first.** A per-write `[Set]` line is debounced per path by the host's `debug` sink (`settings/SchemaSetup.lua`), and an act's line is not, so a write made just before a Defaults press used to print after the act's line, carrying a value the act had replaced. The Options descriptor's `bulkBegin` flushes the pending lines, in write order, before it opens the bracket, and any other line the sink is handed flushes them first too.
- **A library-less load logs none of these.** The schema seam's degradation stub is log-silent: its bracket is a depth for the sweep veto and nothing else, and it takes no reset count. The degraded DebugLog stub has nowhere to show those lines anyway; the writes themselves still land.
...
| `Sl:CliResetAll`, which no KickCD route reaches today | `[Set] reset all: N rows` |

`/kcd reset <path>` writes one row and keeps its one `[Set] <path> = <value>` line. The Spells page's per-spec reset and `/kcd spells resetall` are registry acts, traced as `[Spells]` lines by the registry writer, and are unchanged.
...
```lua
{
    ...
    section  = "general",                    -- Ka0s_KickCD_ConfigChanged section
    ...
    onChange = function(v) ... end,          -- optional reaction, run by the seam AFTER the
                                             --   store and BEFORE the announce; it gets the
                                             --   new value only. Errors propagate.
    get, set = function() ... end, ...       -- optional: the row's own storage, for a row
                                             --   that does not live in db.profile
}
```
...
| `/kcd get <path>` | `Store.FindRow(path)` + `Store.Get` + `formatValue` |
| `/kcd set <path> <value>` | Type-aware parse (clamp numbers, validate dropdown values, parse `r g b [a]` for colors) → `Helpers.SetAndRefresh` → `Store.Set` (store, `onChange`, `Ka0s_KickCD_ConfigChanged`) → `Helpers.RefreshScalars`. A refusal (`false, err, why`) is printed instead of an echo |
| `Defaults` button | `Helpers.RestoreDefaults(panelKey, ctx)` resets every panel row to `def.default` through `Store.ApplyDefault` inside one bracket, runs each `onChange`, fires per-section `Ka0s_KickCD_ConfigChanged`, re-runs the panel's refreshers |
...
A row may declare `valueGate = "<sibling.path>"` for dropdowns whose option list depends on another setting (the canonical case: cast bar `growDirection`'s `RIGHT`/`LEFT` vs `UP`/`DOWN` options gated on `orientation`). The `values` function should re-evaluate the option list against the gate's current value, and the row should also declare `valuesFor = function(gateValue) ... end`, a pure function answering what `values` would return were the gate set to `gateValue` (share one table between the two so the rule is written once). `/kcd set` will surface `(depends on <gate> = <current>; flip <gate> to <other> for <other-options>)` on rejection so a confused user can see why their value was refused AND what to flip to enable it. The flip clauses come from `valuesFor`; a row without it is answered by temporarily swapping the gate's stored value around `values` under `pcall` and restoring it, which is a write to the live profile from a read and is kept only as the fallback.
...
* **Spells** — no schema rows, but it is drawn the way every other page is drawn now:
  ...
  2. a **one-tab strip** under it (`H.TabStrip`, `options-ui-§13`), labeled *Spell list*. Drawn directly rather than through `RenderTabbedSchema`, because there are no rows to partition;
...
All widgets read and write through the schema seam: the Options descriptor's `get` is `Store.Get` and its `set` is `Helpers.SetAndRefresh`, which is `Store.Set` plus a scalar refresh. Blizzard's `Settings.RegisterAddOnSetting` shim was deleted in CR-8 — the canvas widgets never went through Blizzard's Setting object lifecycle, so the shim was dead code. Don't reach for it when adding new UI; use the schema + `Helpers.RenderField` instead.
...
`Helpers.SetAndRefresh` — the addon's single write seam — ends in `Helpers.RefreshScalars`, **never** `RefreshAllPanels`. A value write changes what a widget *shows*; it does not make a row appear or vanish. A structural sweep there would clear and rebuild every rendered page on each committed change, including the page holding the slider or the color swatch the user is still dragging, which AceGUI would take back into its pool mid-gesture. Structural refreshes have their own callers: `NS.RefreshOptionsPanel` on a profile switch, and the `units.focus.link` row's `onChange` (`settings/General.lua`), because the link really does change what the unit pages draw. Every write of that row reaches it (the General → Units tick, `/kcd set units.focus.link`, the page's Defaults, Copy styling), and it runs only when the link actually moves. The tick itself does no refresh; it writes the row through `Helpers.SetAndRefresh` like any widget.
...
- `Dropdown` for `string` rows that declare a `values` list (fixed-choice), or a function returning one. This addon supplies the **key-map** shape — `{ TOP_LEFT = "Top left", ... }` — paired with an explicit `sorting` array where the order matters, because alphabetizing an anchor list scrambles it. `Helpers.LSMValues(mediaType)` is the standard wrapper around LibSharedMedia listings and hands back the **deferred reader**, not the listing — the composers assign it into `values` once at row-declaration time, so a hash there would freeze the list before any media addon has registered — and `Helpers.AnchorValues()` returns the canonical 13-option `<SIDE>_<ALIGN>` / `CENTER` set. As of OptionsWidgets minor 4 the library also reads an **ordered array** of `{ value =, text = }`; either shape works, and note the key is `text` — never `label`.
...
Non-schema action rows are built by `Helpers.InlineButtonPair` (two 50/50 `Button`s — the Master controls tab's "Reset position" beside "Reset all settings", drawn by the composer's own `afterGroup`). General → Units' "Copy styling from Target" left it when it moved onto the tick's line: a button sharing a row with something that is not a button is a `RenderGrid` cell, not a button pair. `Helpers.InlinePair` is **gone**: it existed for exactly one line, the Lock-frame / Debug-console pairing, and that line is two ordinary schema rows now.
...
All six pages declare *how* they draw with `Helpers.SetRenderer(ctx, fn)` and let the **library** own *when*: first show, and again when a refresh marked the page dirty while it was hidden. That second half is what makes a structural refresh from another page work — see the `units.focus.link` row's `onChange` above, which the General → Units tick reaches from another page — and it is why these pages no longer carry the hand-rolled `local rendered = false; OnShow{...}` guard. `SetRenderer` also installs the combat refusal (`options-ui-§2`): the Blizzard AddOns sidebar reaches a panel without going through `OpenOptionsPanel`, so its combat guard was bypassed on exactly the path a user is most likely to take mid-fight.

Spells and Profiles arrived on that seam last (`CX03`), and they came for the **guard**, not for the deferral — they render bespoke bodies with their own lifecycles, not schema rows, so a zero-width `ctx.body` was never their problem. What was their problem is that a hand-parked `OnShow` is reached by the Blizzard AddOns sidebar with nothing in front of it, which is precisely the case the refusal exists for. Both keep the every-show behavior they had, by marking themselves dirty from their own `OnHide` with `Helpers.RefreshPanel(ctx, true)`: Spells hands its entire widget tree back to AceGUI's pool there and would otherwise draw an empty page on the second show, and Profiles re-`Open`s the AceConfigDialog so the page describes the profile that is active now rather than the one that was active the first time it was shown. Neither touches the library's private dirty flag directly — `RefreshPanel` is published precisely so a host does not have to guess at it.
...
The `OnValueChanged` commit is wrapped in `Util.Throttle(50, ...)` so dragging a color slider fires `Ka0s_KickCD_ConfigChanged` at most ~20 times/sec instead of every render frame. Without it the live cast bar / icon grid would re-skin per-frame during a drag, jankily on slower systems. `OnValueConfirmed` (which only fires on Cancel under WoW 12.0's flow) stays immediate so the snap-back is instantaneous.
...
The Options descriptor's `validate` hook runs **`Store.Validate`** (`LibKa0s-Schema-1.0`) at panel-register time. It checks every row for a non-empty `path` (or an Options bound row's `get`/`set`), a known `type` (`bool` / `number` / `string` / `color`), a non-empty **`group`** (`options-ui-§13`; required on every row), and a duplicate `path`, and it resolves every stored path against `NS.DEFAULT_PROFILE`, so a typo'd path that reads and writes nothing is reported too. A row with its own `get` (the console, the minimap button) lives in no defaults tree and is skipped. The **panel** and **section** enums (`general` / `icons` / `castbar` / `label` / `spells` / `profiles`, and `general` / `icons` / `castbar` / `label` / `spells` / `units` / `debug`) are this addon's own vocabulary and are a test now — `tests/test_schema.lua`'s *every row's panel and section are known* — rather than a runtime print. The debug **capture** flag is still session-only and still not a schema row (`NS.State.debug`, toggled via the console header button or `/kcd debug on|off|toggle`). The console **window** is a schema row — `state.debugConsole`, `sessionOnly`, filed under section `general` — but it never reaches SavedVariables either: its own `get`/`set` answer it off `NS.DebugLog`, so the write never touches `db.profile`. Failures print a `schema error` line per offending row through `NS.Util.print` but don't refuse to load — a diagnostic guard rail for future contributors, not a hard gate.

## The write seam (`LibKa0s-Schema-1.0`)

Every settings write lands on **`NS.Settings.Store`**, the `LibKa0s-Schema-1.0` instance `settings/SchemaSetup.lua` builds (the first file of the TOC's settings block). The panel's widgets, `/kcd set` and `/kcd reset`, both resets, Copy styling and the host verbs (`/kcd lock|unlock|toggle`, the launcher menu's two entries, the master switch) all reach it, and every member is dot-called and taken by value (`get = Store.Get`, `findRow = Store.FindRow`). What the library does, in order, for `Store.Set(path, value)`: refuse a path no row declares (`Setting not found: <path>` — **never stored**); `validate`; store (a table is **copied** in; a row with its own `set` gets the value); the `[Set]` line; the row's `onChange` (errors **propagate**); the host's `announce`. What the host adds:

- **`announce`** sends `Ka0s_KickCD_ConfigChanged` with the row's own `section`, through `Helpers.FireConfigChanged`. A `sessionOnly` row or one with its own `get` sends nothing — nothing on the bus renders the console window or the minimap button. **`announceBatch`** sends each distinct section of a `SetMany` once, in first-seen order.
- **The master switch** is the `enabled` row's `onChange` (`settings/General.lua`), which the seam runs **before** it announces, so the addon has stood down or woken by the time the bus message goes out.
- **`writeThrough = { "enabled", "locked" }`** (`NS.Settings.WRITE_THROUGH`, `options-ui-§1` route (a)). Both are composed rows, so a load where the composer is absent has no row for either, and the host verbs that write them still run there. The seam stores a listed path with no row raw (a copy) and announces it with a synthetic `{ path, writeThrough = true }` row, and the host's `announce` takes the disabled hold itself for a written-through `enabled`. Every other row-less path is still refused. A listed path that has a row takes the row.
- **The debounced `[Set]` line** — the host's `debug` sink holds a single-write line per path for 0.3 s so a color drag logs once, and flushes the pending lines before any other line.
- **`resetExempt = { ["global.minimap.shown"] = true }`** — the launcher-§3 sweep veto (see Master controls above).

**With LibKa0s absent** the seam is `settings/SchemaSetup.lua`'s own degradation stub (`NS.Settings.HostSchemaStub`), the one the library's version-2 document prescribes: **write-completing and log-silent**. Reads, writes, `validate`, `onChange`, the announce, the batch and the sweep veto all work; the `[Set]` line, the bracket's tally and the reset count do not. Its refusals are this addon's own words (`locales/enUS.lua`). `tests/test_surface_parity.lua` holds its instance to a live instance and its library half to the major by name.

## Reset helpers

The Master controls tab's "Reset all settings" button — drawn by `H.MasterControls`' returned `afterGroup`, not by this page — funnels through `Helpers.ResetAll`, and **`ResetAll` is now one call**: `Helpers.RestoreAllDefaults`, the library's. That is because the global reset **is a profile reset** (`options-ui-§12`). The descriptor supplies `resetProfile` (`settings/OptionsSetup.lua`), which does `NS.db:ResetProfile()` — AceDB empties the ACTIVE profile only, `aceDBDefaults()` merges `NS.DEFAULT_PROFILE` back over it, and `Database:OnProfileChanged` folds legacy units, migrates spec keys, **re-seeds every spec's spell list** through `BuildSpells` and refreshes, exactly as it does for a profile switch. The library's own row walk narrows to the `sessionOnly` rows before calling it, because everything else is about to be discarded whole.

Three things follow, and each was a separate call in this helper until this pass:

* **The anchors and the Focus `link` flag come back for free**, since both live in the profile, so `Helpers.ResetAllPositions`, which used to put every anchor back on its own, is gone. Its sibling `Helpers.RestoreUnitLinks` is gone: `units.focus.link` is a schema row, so the General page's **Defaults** and `/kcd reset units.focus.link` restore it too.
* **So do the spell lists.** `db.profile.spells` goes with the profile, so `Database:ResetAllSpells` left this path too; it still backs `/kcd spells resetall`.
* **The descriptor's `afterRestoreAll` hook is gone**, replaced by `resetProfile` at `LibKa0s-Options-1.0` minor 9. `ResetAll` used to re-run the hook's work itself (KCD-R-04), which made the hook look optional while the library's own footer Defaults control depended on it; there is no hook to duplicate now.

The slash command `/kcd resetall` shares this same helper — the popup and the CLI cannot diverge. Profiles rows are intentionally skipped (`descriptor.skipRestoreAll`) because the AceDBOptions UI has its own destructive controls and resetting them would delete user data — and the profile **list** is untouched either way: `ResetProfile` empties one profile, it does not remove any.

`Helpers.ResetIconPosition` is the corresponding helper for the "Reset position" button and `/kcd resetposition` — it walks `NS.Units.LIST` and writes each unit's `db.profile.units.<unit>.anchors.icons` from `NS.DEFAULT_PROFILE.units.<unit>.anchors.icons`, so the default coordinates live in one place — literally one: a unit with no default there is skipped, and if the tree is absent altogether it writes nothing and publishes nothing, rather than falling back to a coordinate of its own (the fallback it used to carry disagreed with `defaults/Profile.lua` by 300 px in the opposite direction). It moves the icon grids only; a cast bar set to move freely keeps its `anchors.castbar`, and an anchored bar follows its grid anyway. It fires `Ka0s_KickCD_ConfigChanged { section = "general" }` once, and only that — IconGrid's `general` branch already re-anchors every enabled unit's grid from its own anchor, and firing `icons` would just be a wasted relayout. It writes the anchor directly rather than through `NS.Units.SetAnchor`, the anchors' owner, and [ARCHITECTURE.md](ARCHITECTURE.md#settings-schema) lists it as one of their writers (`architecture-§5` named non-setting state).
