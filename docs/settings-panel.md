...
Every settings write lands on **`NS.Settings.Store`**, the `LibKa0s-Schema-1.0` instance `settings/SchemaSetup.lua` builds (the first file of the TOC's settings block). The panel's widgets, `/kcd set` and `/kcd reset`, both resets, Copy styling and the host verbs (`/kcd lock|unlock|toggle`, the launcher's click, the master switch) all reach it, and every member is dot-called and taken by value (`get = Store.Get`, `findRow = Store.FindRow`). What the library does, in order, for `Store.Set(path, value)`: refuse a path no row declares (`Setting not found: <path>` — **never stored**); `validate`; store (a table is **copied** in; a row with its own `set` gets the value); the `[Set]` line; the row's `onChange` (errors **propagate**); the host's `announce`. What the host adds:
