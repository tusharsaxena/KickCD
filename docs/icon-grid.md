...
## Seeding a rebuilt icon

`BuildActiveList` releases the whole pool and acquires a fresh button per spell, so every rebuilt icon needs a first state before any `Ka0s_KickCD_SpellState` reaches it. It pulls that state from `Cooldowns:StateFor(spellID)` — a **read-only query** of the entry in Cooldowns' `watched` table, never mutated by the caller — and falls back to the file-scope `READY_SEED` (`{ ready = true, start = 0, duration = 0 }`, one shared table, so an unwatched icon costs no allocation) when Cooldowns holds nothing for the spell. Both modules rebuild on `PLAYER_ENTERING_WORLD`, `SPELLS_CHANGED`, `TRAIT_CONFIG_UPDATED` and a profile change, and CallbackHandler runs their handlers in `pairs` order; when Cooldowns goes first its `SPELL_STATE` lands on the pool IconGrid is about to release, and it does not emit again until the spell's state changes. Seeding ready used to leave an interrupt on cooldown showing ready after a talent swap, a pet summon or a `/reload`; the pull makes the icon correct whichever handler runs first. It is a query, not a second sender — the bus stays Cooldowns → IconGrid (architecture-§4; see [message-bus.md](message-bus.md#ka0s_kickcd_spellstate-payload)). Guarded by `tests/test_icongrid_buildlist.lua`.

## Per-unit instances

Each instance's cast state is driven by ONE cast-filter frame (`Util.NewUnitCastFilter`), built the first time the unit enables and armed / disarmed across every later disable, suspend and re-enable rather than rebuilt. Its fixed event set is the file-scope `ICON_CAST_ROUTES`: `UNIT_SPELLCAST_START` / `_STOP` / `_FAILED` / `_INTERRUPTED` / `_CHANNEL_START` / `_CHANNEL_STOP` / `_INTERRUPTIBLE` / `_NOT_INTERRUPTIBLE` / `_EMPOWER_START` / `_EMPOWER_UPDATE` / `_EMPOWER_STOP`, every one routed to `IconGrid:OnUnitCastEvent`, which re-runs the unit's visibility and glows.

Each unit's instance ends its `Layout()` pass by emitting `Ka0s_KickCD_GridLayout { unit, gridFrame, primaryIcon, width, height }` (see [message-bus.md](message-bus.md#ka0s_kickcd_grid_layout-payload)) — `Castbar` filters on `payload.unit` before re-anchoring/auto-sizing its same-unit instance. Positioning (`anchors.icons`) is always per-unit, even when a linked focus instance renders with target's `icons` styling table. The optional identity label is a separate concern owned by `modules/UnitLabel.lua` (attaches to the grid *or* cast bar per `label.style.attach`): only `label.text` stays per-unit — `label.show` and `label.style` both follow the focus link — see [module-map.md](module-map.md).
