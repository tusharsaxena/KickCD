...
-- ── Test mode ───────────────────────────────────────────────────────────────
--
-- Test mode (options-ui-§15) puts the placeholder bar on screen while the bar
-- stays LOCKED. It shows; it does not make anything draggable.

test("test mode puts the PREVIEW on screen while locked, with no cast", function()
    -- red under: ApplyLock's no-cast arm reading `locked` instead of State.IsPreviewing
    local NS, _, Castbar = enabled()
    makeVisible(NS)
    NS.db.profile.visibility = "target_casting_interruptible"   -- a mode that would hide it
    local inst = Castbar:GetInstance("target")
    Castbar:EnsureFrame(inst)
    Castbar:ApplyLock(inst)
    assertFalse(inst.frame:IsShown(), "precondition: locked, no cast, no test mode -> hidden")
    NS.State.testMode = true
    Castbar:ApplyLock(inst)
    assertTrue(inst.frame:IsShown(), "test mode must show the locked bar's preview")
    assertEqual(inst.frame.bar.interruptible:GetValue(), 0.5, "the preview sits mid-bar")
    assertEqual(inst.frame:GetAlpha(), 1)
end)

test("Stop leaves the PREVIEW on screen in test mode while locked", function()
    -- red under: Stop reading `locked` instead of State.IsPreviewing
    local NS, _, Castbar = enabled()
    makeVisible(NS)
    local inst = Castbar:GetInstance("target")
    Castbar:Start(inst, castRecord())
    NS.State.testMode = true
    Castbar:Stop(inst)
    assertTrue(inst.frame:IsShown(), "the test-mode bar must remain visible")
    assertEqual(inst.frame.bar.interruptible:GetValue(), 0.5, "the preview sits mid-bar")
end)

test("test mode does not make a FREE cast bar draggable while locked", function()
    -- Only Lock frame gates the drag.
    -- red under: ApplyLock's dragAllowed reading State.IsPreviewing
    local NS, _, Castbar = enabled()
    makeVisible(NS)
    NS.Units.Castbar("target").anchorMode = "FREE"
    local inst = Castbar:GetInstance("target")
    Castbar:EnsureFrame(inst)
    local mouse = {}
    rawset(inst.frame, "EnableMouse", function(_, on) mouse[#mouse + 1] = on end)
    NS.State.testMode = true
    Castbar:ApplyLock(inst)
    assertEqual(mouse[#mouse], false, "test mode made the locked bar draggable")
    assertFalse(inst.frame.dragHint:IsShown(), "no drag hint on a bar that cannot drag")
    NS.db.profile.locked = false
    Castbar:ApplyLock(inst)
    assertEqual(mouse[#mouse], true, "unlocking still makes it draggable")
    assertTrue(inst.frame.dragHint:IsShown())
end)

test("ticking Test mode repaints the grid and the bar, and unticking clears them", function()
    -- End to end through the row: the write announces "general", and IconGrid
    -- and Castbar re-apply off it. Locked throughout, with no cast anywhere.
    -- red under: the SESSION_PATHS testMode set not firing CONFIG_CHANGED
    local NS, mocks, Castbar = enabled()
    makeVisible(NS)
    NS.db.profile.visibility = "target_casting_interruptible"
    mocks.UnitExists = function() return false end
    local H = NS.Settings.Helpers
    H.SetAndRefresh("locked", true)
    local bar = Castbar:GetInstance("target")
    local grid = NS:GetModule("IconGrid"):GetInstance("target")
    Castbar:EnsureFrame(bar)
    assertFalse(bar.frame:IsShown(), "precondition: the bar is hidden")
    assertFalse(grid.grid:IsShown(), "precondition: the grid is hidden")

    H.SetAndRefresh("state.testMode", true)
    assertTrue(bar.frame:IsShown(), "test mode must show the bar preview")
    assertTrue(grid.grid:IsShown(), "test mode must show the grid")

    H.SetAndRefresh("state.testMode", false)
    assertFalse(bar.frame:IsShown(), "turning test mode off must hide the bar again")
    assertFalse(grid.grid:IsShown(), "turning test mode off must hide the grid again")
end)
