-- Copied from LibKa0s v1.58.0 tests/mock_menu.lua, byte for byte below this header. The library keeps it
-- repo-local rather than in the kit, and its version-4 document tells a host that pins its menu
-- entries to install its own fake modeled on that file; this is that file, unedited, so a case here
-- drives the same stand-in the library's own Launcher suite does. Installed per instance by
-- tests/test_launcher.lua and tests/test_disabled.lua (`menuFor`).
--
