...
-- Every entry was already in the `<code>/<variable>` form, which is the narrow SPELLING and is not
-- the narrow SCOPE: at the top level those three names were silenced in all 93 files, including the
-- ones with no business producing them. Removing the three lines took `luacheck .` from 0/0 to
-- SIXTY-ONE warnings, and thirty-two of them were defects rather than conventions — twenty-nine
-- files opening `local addonName, NS = ...` over a folder name they never read, two receivers named
-- and never read in the test tree, and one `NS.Slash:PrintHelp` forwarder nothing called, which the
-- blanket had made look like the third member of a trio. All thirty-two are gone from the source as
-- of `M4c-06`; the twenty-nine that remain are receivers a calling convention forces, answered by
-- nine per-file stanzas.
--
-- The measurement that says the difference is real, and it is not the obvious one. A dead argument
-- under a NEW name always reported here, because the old entries named their variables — so
-- `deadArg` goes red under both configs and proves nothing. What the old config swallowed is the
-- NAME it listed, everywhere: `function NS.Util:DeadProbe()` appended to core/Util.lua reports
-- `unused argument 'self'` under the config this gate guards, and reports nothing under the
-- blanket. The twenty-nine `addonName` headers are that same fact at scale — they sat under a green
-- 0/0 for the life of the addon.
--
...
test("lintconfig: .luacheckrc sets no top-level ignore", function()
    ...
    if ignore ~= nil then
        ...
        fail(".luacheckrc sets a top-level `ignore` of { " .. table.concat(shown, ", ") .. " }. "
            .. "A blanket ignore silences the code in all 93 files, including the ones with no "
            .. "business producing it, so it reads as coverage and provides none (lint-§1, "
            .. "`M4-11`) — and naming the variable does not save it, because the SCOPE is what is "
            .. "wrong. Move each code into a `files[...]` stanza naming the file that earns it, or "
            .. "put a `-- luacheck: ignore <code>` beside the single line that needs it", 2)
    end
end)
...
-- Vendored trees are not ours to annotate — libs/ is linted in the LibKa0s repo and tests/_kit/ is
-- a byte copy of that repo's testkit — and this file quotes the forbidden directive in order to
-- forbid it.
...
test("lintconfig: no source file carries a bare inline luacheck ignore", function()
    ...
    for _, path in ipairs(trackedLua()) do
        ...
        for line in fh:lines() do
            ...
            -- all twenty-nine of its live suppressions are per-file stanzas.
