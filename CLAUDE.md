...
Bundles [LibKa0s](https://github.com/tusharsaxena/LibKa0s) v1.46.1 (MIT). That line is the answer to
...
LibDataBroker-1.1, LibDBIcon-1.0, LibSharedMedia-3.0 and LibStub. Everything ships inside the addon zip from `libs/`; there is no build
...
- **Eleven LibKa0s majors are adopted.** Ten have a setup file each: `core/CoreSetup.lua` (printer + secret seam + the one `NS.MakeCloseButton` wrapper), `core/EnvSetup.lua` (the TOC manifest — `NS.Meta` and `NS.Version`, replacing three inline `C_AddOns` ladders), `core/PoolSetup.lua` (the keyed widget pool behind the icon grid — `NS.Pool`, spellID-keyed, adopted at `LibKa0s-Pool-1.0` minor 2), `core/MediaSetup.lua` (the shared icon set and the monospace face — TOC position is load-bearing, ahead of `core/Constants.lua`), `core/DebugLogSetup.lua` (console), `core/LauncherSetup.lua` (the launcher — `NS.Launcher`, ONE LibDataBroker object behind the minimap button and the broker plugin; left click toggles the lock, which is rung (b) of `launcher-§2`), `core/LifecycleSetup.lua` (the stand-down latch — `NS.Lifecycle`, the `disabled` and `perf` holds), `core/PerfSetup.lua` (A/B capture), `settings/Slash.lua` (dispatcher + schema CLI), `settings/OptionsSetup.lua` (settings panel). The eleventh, `LibKa0s-Widgets-1.0`, has **no** setup file: `settings/Spells.lua` resolves it inline for the spell list's drag-reorder (`options-ui-§18`) and degrades to a non-draggable list without it. **Never edit `libs/`** — a library problem is fixed upstream in `../LibKa0s` and re-vendored. The vendor gate is **two** diffs, run before every commit: `diff -r --strip-trailing-cr ../LibKa0s/LibKa0s libs/LibKa0s` (content — MUST be empty; anything here is a fork) and `diff -r ../LibKa0s/LibKa0s libs/LibKa0s` (bytes — SHOULD be empty; a difference here alone is line-ending drift, renormalized, never fixed by editing `libs/`). Same pair for `../LibKa0s/testkit` ↔ `tests/_kit`. See [docs/testing.md](docs/testing.md).
...
- **`NS.Settings.Schema` is the single source of truth** for every option (one row → UI widget + `/kcd get|set|list` + Defaults reset). `NS` is the private addon namespace (`local _, NS = ...`, or `local addonName, NS = ...` in the five files that hand the folder name to a vendored LibKa0s payload) — there is no `_G.KickCD`.
