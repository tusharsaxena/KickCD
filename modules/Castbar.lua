...
-- A third took the event and message handlers (OnCastStart ... OnGridLayout) to
-- modules/Castbar_Events.lua (#24, KC-ATS-01). What remains is the instance
-- model, the frame build (EnsureFrame), the per-cast paint (RenderCast), the
-- OnUpdate loop and the lifecycle (EnableUnit ... OnDisable). The handler
-- descriptions below still hold; the code that implements them moved.
--
...
--                         OnInterruptibilityChanged (modules/Castbar_Events.lua)
...
-- For modules/Castbar_Events.lua, whose handlers resolve an instance and gate it
-- exactly as they did when they lived here. Same file-locals, same identities:
-- `instances` is the one table this file mutates, never reassigned, so the peel
-- holds the live map rather than a copy. Taken off this table once, at load.
Castbar.Instances           = instances
Castbar.Cfg                 = cfg
Castbar.ForEachEnabled      = forEachEnabled
Castbar.IsVisible           = isVisible
Castbar.ApplyVisibilityMask = ApplyVisibilityMask
