...
![Tests](https://img.shields.io/badge/Tests-949%2F949_passing-green)
...
So the first job is positioning. Drag the grids and bars where you want them, then run `/kcd lock`; one lock covers every unit, and `/kcd unlock` frees them again. To check the layout later without unlocking, tick "Test mode" on General → Master controls or type `/kcd test`: the placeholders come back but nothing moves, and entering combat switches it off. A grid always drags while unlocked, but a cast bar drags only if you've set it to move freely, since by default it is anchored to the grid's primary icon and travels along with it. `/kcd resetposition` brings the target grid back to the middle of the screen if you push it off the edge. `/kcd resetall` is not the bigger version of that — it restores every settings page and every spec's spell list to defaults as well as the positions, so it is a fresh start, not a nudge.
...
| Can I see where everything sits without unlocking? | Yes. Tick "Test mode" on General → Master controls, or type `/kcd test`. The grids and cast bars show with placeholder content and stay locked. It goes off when you untick it, when you enter combat, or when you reload. |
...
| The icon grid won't drag. | It's locked. `/kcd unlock`, drag, `/kcd lock`. If unlocking doesn't seem to take, run `/kcd toggle`. Test mode shows the grid but doesn't unlock it. |
